...
| 파티션 한도 (4,000개) 주의 | **약 167일분** (초과 주의) | 약 11년분 | 약 333년분 |
...
> **주의**: HOUR 파티션은 파티션 수가 매우 빠르게 증가합니다. BigQuery의 테이블당 파티션 한도는 4,000개이므로, 약 167일(5.5개월) 분량의 데이터만 유지할 수 있습니다.
...
5. **HOUR 파티션의 파티션 한도 주의**: 테이블당 최대 4,000개 파티션이므로 HOUR 파티션은 약 167일분만 유지할 수 있습니다.
